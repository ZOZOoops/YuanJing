...
class Yuanjing_nologin_view(unittest.TestCase, Base_Keys,Log):

    base_url = 'https://www.yuanjingio.com/'

    def setUp(self) -> None:
        self.driver = self.chooseBrowser('edge')
    ...
    def test_005_register(self): # 点击注册按钮
    ...
    def test_006_login(self): # 未登录情况下点击登录正常
        self.click((By.CSS_SELECTOR,'div.yuanjing-header3 > div > div:nth-child(2) > div:nth-child(3)'))
        self.implicilty_wait(2)
        ele = self.locate((By.CSS_SELECTOR,'div.page-section.center.third-login-wrapper > div > h2'))
        self.assertEqual('欢迎登录元境',ele.text,msg='校验登录错误')
